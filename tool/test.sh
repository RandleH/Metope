...
source setup.env && rebuild "-DCMAKE_BUILD_TYPE=Debug -DLOG_LEVEL=2" > ${PRJ_TOP}/build.log 2>&1
...
source setup.env native && rebuild "-DCMAKE_BUILD_TYPE=Debug" > ${PRJ_TOP}/build.log 2>&1
if [ ! -e "${PRJ_TOP}/build/model1.elf" ]; then
    echo ".elf file was NOT found!"
    exit 1
fi
